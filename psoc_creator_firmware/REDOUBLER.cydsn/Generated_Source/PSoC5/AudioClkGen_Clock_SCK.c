/*******************************************************************************
* File Name: AudioClkGen_Clock_SCK.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_Start
********************************************************************************
*
* Summary:
...
void AudioClkGen_Clock_SCK_Start(void) 
{
    ...
    AudioClkGen_Clock_SCK_CLKEN |= AudioClkGen_Clock_SCK_CLKEN_MASK;
	AudioClkGen_Clock_SCK_CLKSTBY |= AudioClkGen_Clock_SCK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_Stop
********************************************************************************
*
* Summary:
...
void AudioClkGen_Clock_SCK_Stop(void) 
{
    ...
    AudioClkGen_Clock_SCK_CLKEN &= (uint8)(~AudioClkGen_Clock_SCK_CLKEN_MASK);
	AudioClkGen_Clock_SCK_CLKSTBY &= (uint8)(~AudioClkGen_Clock_SCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((AudioClkGen_Clock_SCK_CLKEN & AudioClkGen_Clock_SCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = AudioClkGen_Clock_SCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = AudioClkGen_Clock_SCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* AudioClkGen_Clock_SCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(AudioClkGen_Clock_SCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(AudioClkGen_Clock_SCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = AudioClkGen_Clock_SCK_GetSourceRegister();
    uint16 oldDivider = AudioClkGen_Clock_SCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = AudioClkGen_Clock_SCK_CLKEN & AudioClkGen_Clock_SCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(AudioClkGen_Clock_SCK_DIV_PTR, clkDivider);
                AudioClkGen_Clock_SCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                AudioClkGen_Clock_SCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(AudioClkGen_Clock_SCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((AudioClkGen_Clock_SCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(AudioClkGen_Clock_SCK_DIV_PTR, clkDivider);
				AudioClkGen_Clock_SCK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(AudioClkGen_Clock_SCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Clock_SCK_MOD_SRC |= modeBitMask & (uint8)AudioClkGen_Clock_SCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Clock_SCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(AudioClkGen_Clock_SCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Clock_SCK_MOD_SRC & (uint8)(AudioClkGen_Clock_SCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = AudioClkGen_Clock_SCK_GetDividerRegister();
    uint8 oldSrc = AudioClkGen_Clock_SCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        AudioClkGen_Clock_SCK_MOD_SRC |= CYCLK_SSS;
        AudioClkGen_Clock_SCK_MOD_SRC =
            (AudioClkGen_Clock_SCK_MOD_SRC & (uint8)(~AudioClkGen_Clock_SCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Clock_SCK_MOD_SRC & AudioClkGen_Clock_SCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Clock_SCK_PHASE = clkPhase & AudioClkGen_Clock_SCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Clock_SCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Clock_SCK_PHASE & AudioClkGen_Clock_SCK_PHASE_MASK;
}
