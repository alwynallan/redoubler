...
void Opamp_2_Stop(void) 
{
    ...
    Opamp_2_PM_ACT_CFG_REG &= (uint8)(~Opamp_2_ACT_PWR_EN);

    /* Disable power to buffer in alternative active mode template */
    Opamp_2_PM_STBY_CFG_REG &= (uint8)(~Opamp_2_STBY_PWR_EN);
    
    /* Disable negative charge pumps for ANIF only if all ABuf is turned OFF */
    if(Opamp_2_PM_ACT_CFG_REG == 0u)
    {
        Opamp_2_PUMP_CR1_REG &= (uint8)(~(Opamp_2_PUMP_CR1_CLKSEL | Opamp_2_PUMP_CR1_FORCE));
    }
}
...
void Opamp_2_SetPower(uint8 power) 
{
        ...
        Opamp_2_CR_REG &= (uint8)(~Opamp_2_PWR_MASK);
        Opamp_2_CR_REG |= power & Opamp_2_PWR_MASK;      /* Set device power */
    #else
        CYASSERT(Opamp_2_HIGHPOWER == power);
