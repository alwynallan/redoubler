...
extern uint8 volumeUpdateReceived;
#define OUTPUT_VOLUME_FEATURE_UNIT   0x02
#define INPUT_VOLUME_FEATURE_UNIT    0x06

volatile uint8 USBFS_currentCH1InputVolume[USBFS_VOLUME_LEN];
volatile uint8 USBFS_currentCH2InputVolume[USBFS_VOLUME_LEN];
volatile uint8 USBFS_currentTmpCH1InputVolume[USBFS_VOLUME_LEN];
volatile uint8 USBFS_currentTmpCH2InputVolume[USBFS_VOLUME_LEN];

#ifndef ENABLE_DIGITAL_AUDIO_OUT_ONLY
volatile uint8 USB_inputMute;
volatile uint8 USB_inputVolume[USBFS_VOLUME_LEN];
#endif 
/* `#END` */
                ...
                case USBFS_GET_CUR:
                    ...
                    else if(CY_GET_REG8(USBFS_wValueHi) == USBFS_VOLUME_CONTROL)
                    {
                        /* `#START VOLUME_CONTROL_GET_REQUEST` Place multi-channel handler here */
    						...
    						switch(CY_GET_REG8(USBFS_wIndexHi))
    	                    {
                                case INPUT_VOLUME_FEATURE_UNIT:
	                            USBFS_currentTD.pData  = USB_inputVolume;
                                if(CY_GET_REG8(USBFS_wValueLo) == 1)	/* channel 1 input volume update to host */
    							{
                            		USBFS_currentTD.pData  = &USBFS_currentCH1InputVolume[0];							
    							}
    							else if(CY_GET_REG8(USBFS_wValueLo) == 2)	/* channel 2 input volume update to host */
    							{
                            		USBFS_currentTD.pData  = &USBFS_currentCH2InputVolume[0];							
    							}
                                break;
                                
    							case OUTPUT_VOLUME_FEATURE_UNIT:
                ...
                case USBFS_SET_CUR:
                    ...
                    if(CY_GET_REG8(USBFS_wValueHi) == USBFS_MUTE_CONTROL)
                    {
                        ...
                        switch(CY_GET_REG8(USBFS_wIndexHi))
                        {
                            ...
                            case INPUT_VOLUME_FEATURE_UNIT:
                                 /* Entity ID Control Selector is MUTE */
                                USBFS_currentTD.pData  = &USB_inputMute;
                            break;    
                        }
                        requestHandled   = USBFS_InitControlWrite();
                        break;
                    ...
                    else if(CY_GET_REG8(USBFS_wValueHi) == USBFS_VOLUME_CONTROL)
                    {
                        ...
                        switch(CY_GET_REG8(USBFS_wIndexHi))
                        {
                            ...
                            case INPUT_VOLUME_FEATURE_UNIT:
                                USBFS_currentTD.pData  = USB_inputVolume;
                                if(CY_GET_REG8(USBFS_wValueLo) == 1)	/* channel 1 input volume update from host */
    							{
    		                        USBFS_currentTD.pData  = &USBFS_currentCH1InputVolume[0];							
    							}
    							else if(CY_GET_REG8(USBFS_wValueLo) == 2) /* channel 2 input volume update from host */
    							{
    		                        USBFS_currentTD.pData  = &USBFS_currentCH2InputVolume[0];								
    							}
                            break;    
