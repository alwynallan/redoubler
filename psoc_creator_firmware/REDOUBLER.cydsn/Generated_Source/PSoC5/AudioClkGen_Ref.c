...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_Start
********************************************************************************
*
* Summary:
...
void AudioClkGen_Ref_Start(void) 
{
    ...
    AudioClkGen_Ref_CLKEN |= AudioClkGen_Ref_CLKEN_MASK;
	AudioClkGen_Ref_CLKSTBY |= AudioClkGen_Ref_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_Stop
********************************************************************************
*
* Summary:
...
void AudioClkGen_Ref_Stop(void) 
{
    ...
    AudioClkGen_Ref_CLKEN &= (uint8)(~AudioClkGen_Ref_CLKEN_MASK);
	AudioClkGen_Ref_CLKSTBY &= (uint8)(~AudioClkGen_Ref_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((AudioClkGen_Ref_CLKEN & AudioClkGen_Ref_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = AudioClkGen_Ref_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = AudioClkGen_Ref_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* AudioClkGen_Ref__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(AudioClkGen_Ref_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(AudioClkGen_Ref_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = AudioClkGen_Ref_GetSourceRegister();
    uint16 oldDivider = AudioClkGen_Ref_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = AudioClkGen_Ref_CLKEN & AudioClkGen_Ref_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(AudioClkGen_Ref_DIV_PTR, clkDivider);
                AudioClkGen_Ref_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                AudioClkGen_Ref_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(AudioClkGen_Ref_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((AudioClkGen_Ref_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(AudioClkGen_Ref_DIV_PTR);
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Ref_MOD_SRC |= modeBitMask & (uint8)AudioClkGen_Ref_MODE_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Ref_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(AudioClkGen_Ref_MODE_MASK));
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Ref_MOD_SRC & (uint8)(AudioClkGen_Ref_MODE_MASK);
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = AudioClkGen_Ref_GetDividerRegister();
    uint8 oldSrc = AudioClkGen_Ref_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        AudioClkGen_Ref_MOD_SRC |= CYCLK_SSS;
        AudioClkGen_Ref_MOD_SRC =
            (AudioClkGen_Ref_MOD_SRC & (uint8)(~AudioClkGen_Ref_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Ref_MOD_SRC & AudioClkGen_Ref_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: AudioClkGen_Ref_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    AudioClkGen_Ref_PHASE = clkPhase & AudioClkGen_Ref_PHASE_MASK;
}


/*******************************************************************************
* Function Name: AudioClkGen_Ref_GetPhase
********************************************************************************
*
* Summary:
...
{
    return AudioClkGen_Ref_PHASE & AudioClkGen_Ref_PHASE_MASK;
}
