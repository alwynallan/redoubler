...
uint8 Comp_4_initVar = 0u;

/* Internal functions definitoin */
static void Comp_4_trimAdjust(uint8 nibble) ;

/* static Comp_4_backupStruct  Comp_4_backup; */
#if (CY_PSOC5A)
    static Comp_4_LOWPOWER_BACKUP_STRUCT  Comp_4_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_4_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_4_Init
********************************************************************************
*
* Summary:
...
void Comp_4_Init(void) 
{
    ...
    Comp_4_SetSpeed(Comp_4_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_4_DEFAULT_HYSTERESIS == 0u )
        Comp_4_CR |= Comp_4_HYST_OFF;
    #else
        Comp_4_CR &= (uint8)(~Comp_4_HYST_OFF);
    #endif /* Comp_4_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_4_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_4_CR &= (uint8)(~Comp_4_PWRDWN_OVRD);
        #else 
            Comp_4_CR |= Comp_4_PWRDWN_OVRD;
        #endif /* Comp_4_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_4_CR |= Comp_4_MX_AO;

    /* Set default sync */
    Comp_4_CLK &= (uint8)(~Comp_4_SYNCCLK_MASK);
    #if ( Comp_4_DEFAULT_BYPASS_SYNC == 0u )
        Comp_4_CLK |= Comp_4_SYNC_CLK_EN;
    #else
        Comp_4_CLK |= Comp_4_BYPASS_SYNC;
    #endif /* Comp_4_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_4_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_4_PWRMGR |= Comp_4_ACT_PWR_EN;
    Comp_4_STBY_PWRMGR |= Comp_4_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_4_restoreReg == 1u)
        {
            Comp_4_CR = Comp_4_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_4_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Comp_4_initVar: Is modified when this function is called for the 
...
void Comp_4_Stop(void) 
{
    ...
    Comp_4_PWRMGR &= (uint8)(~Comp_4_ACT_PWR_EN);
    Comp_4_STBY_PWRMGR &= (uint8)(~Comp_4_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_4_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_4_lowPowerBackup.compCRReg = Comp_4_CR;
        Comp_4_CR = Comp_4_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_4_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_4_SetSpeed(uint8 speed) 
{
    ...
    Comp_4_CR = (Comp_4_CR & (uint8)(~Comp_4_PWR_MODE_MASK)) |
                           (speed & Comp_4_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_4_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_4_TR = Comp_4_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_4_TR0 = Comp_4_HS_TRIM_TR0;
            Comp_4_TR1 = Comp_4_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_4_TR = Comp_4_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_4_TR0 = Comp_4_LS_TRIM_TR0;
            Comp_4_TR1 = Comp_4_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_4_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_4_WRK & Comp_4_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_4_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_4_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_4_WRK & Comp_4_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_4_TR |= Comp_4_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_4_TR0 |= Comp_4_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_4_TR |= Comp_4_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_4_TR1 |= Comp_4_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_4_WRK & Comp_4_CMP_OUT_MASK) != cmpState)
        {
...
uint16 Comp_4_ZeroCal(void) 
{
    ...
    Comp_4_SW0 = 0x00u;
    Comp_4_SW2 = 0x00u;
    Comp_4_SW3 = tmpSW3 & (uint8)(~Comp_4_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_4_CR;
    Comp_4_CR |= (Comp_4_CAL_ON | Comp_4_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_4_TR = Comp_4_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_4_TR0 = Comp_4_DEFAULT_CMP_TRIM;
        Comp_4_TR1 = Comp_4_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_4_CR & Comp_4_PWR_MODE_MASK) == Comp_4_PWR_MODE_FAST)
    {
        Comp_4_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_4_trimAdjust(1u);
		Comp_4_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_4_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_4_TR1 << 8u) | (Comp_4_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_4_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_4_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_4_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_4_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
